        ...
        <div>
          {steps.map(([n, title, desc]) => (
            <div key={n} style={{
              ...
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--accent)" }}>{n}</div>
              <h3 style={{
                fontFamily: "var(--font-sans)", fontSize: 28, fontWeight: 500,
                letterSpacing: "-0.02em", margin: 0, color: "var(--text)",
              }}>{title}</h3>
              <p className="method-desc" style={{ fontFamily: "var(--font-sans)", fontSize: 15, color: "var(--dim)", margin: 0, lineHeight: 1.55 }}>{desc}</p>
            </div>
          ))}
